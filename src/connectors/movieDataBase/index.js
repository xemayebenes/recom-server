...
export default ({ apiKey = 'aef9a2eff559d403c028eac5d3acf757' }) => {
  ...
  connector.generateSearchFilmUriRequest = (searchText, language) =>
    `${DATA_REQUEST_URL}${SEARCH_PATH}api_key=${apiKey}&language=${language}&query=${encodeURI(
      searchText
    )}`;

  connector.generateSearchTvUriRequest = (searchText, language) =>
    `${DATA_REQUEST_URL}${SEARCHTV_PATH}api_key=${apiKey}&language=${language}&query=${encodeURI(
      searchText
    )}`;
  ...
  connector.transformFilmData = list =>
    list.results ? list.results.map(connector.normalizeItemData) : [];
  connector.transformSerieData = list =>
    list.results ? list.results.map(connector.normalizeTVData) : [];
  connector.transformVideoData = list =>
    list.results
